...
def main(wf):
    ...
    if "g" in prefixes:
        ...
        if games is None:
            log.debug("Games loading...")

        elif "id" in prefixes:
            gm = list(filter(lambda x: x.get('id') == query, games["data"]['items']))
            if not gm:
                wf.add_item(
            ...
            c = games['data']['count']
            dat = wf.filter(query, games['data']['items'], _filter)
            wf.add_item(
                title="Поиск по играм",
